...
$(document).ready(function () {
    ...
    // ⚡ Bolt: Cache static container elements outside event handlers to avoid redundant O(N) DOM queries
    let $content = $("#content");
    ...
    PROPS.projects.forEach(function (el, i) {
        // ⚡ Bolt: Add loading="lazy" to defer off-screen image loading
        cards += `<div class="card">
                <div class="card-image">
                    ...
                    <a class="btn-floating halfway-fab waves-effect waves-light light-blue" href="${el.url}" target="_blank" aria-label="${el.name} link"><i class="material-icons">language</i></a>
                </div>
    ...
    // ⚡ Bolt: Replaced O(n log n) map().sort().map() chain with O(n) Fisher-Yates algorithm
    // to avoid intermediate array allocations and inherent statistical bias.
    ...
    shuffled.forEach(function (e) {
        ...
        // ⚡ Bolt: Removed redundant `let years = (new Date()).getFullYear() - e.year;`
        // which caused unnecessary object instantiation in loop (dead code).
    ...
    PROPS.cohorts.forEach(function (e, i) {
        // ⚡ Bolt: Add loading="lazy" to defer off-screen cohort image loading
        imgsHTML += `<li>
                        <img class="cohort materialboxed" src="img/cohorts/${e}" loading="lazy" alt="${e.split('.')[0]}">
                    </li>`;
    });
