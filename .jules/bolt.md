...
## 2024-05-29 - [Avoid map-sort-map array shuffle]
**Learning:** The `map().sort().map()` pattern for shuffling arrays creates unnecessary intermediate objects, triggers multiple array allocations, and has O(n log n) overhead due to sorting.
**Action:** Use the Fisher-Yates algorithm for in-place array shuffling instead. It operates in O(n) time and avoids intermediate object allocations.
