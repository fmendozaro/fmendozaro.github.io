...
**Learning:** Using chained `.map().sort().map()` for array shuffling introduces unnecessary O(n log n) overhead and allocates multiple intermediate arrays. This is an anti-pattern, particularly in loops or initialization sequences.
**Action:** Use the Fisher-Yates algorithm for in-place array shuffling instead. It operates in O(n) time and O(1) space, avoiding sorting overhead and temporary objects.
