...
## 2026-06-03 - [Inefficient Array Shuffling]
**Learning:** Using chained `.map().sort().map()` to shuffle arrays allocates intermediate arrays and operates at O(n log n) overhead, leading to poor performance compared to an in-place shuffle.
**Action:** Use the Fisher-Yates algorithm for in-place array shuffling instead of `map().sort().map()` chains to avoid O(n log n) overhead and intermediate array allocations.
