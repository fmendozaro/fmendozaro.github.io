...
## 2024-03-27 - [Inefficient Array Shuffling]
**Learning:** Using `array.map().sort().map()` with `Math.random()` to shuffle an array is O(n log n), creates multiple intermediate arrays resulting in memory allocation overhead, and suffers from statistical bias. While the array here is small, this is a known JavaScript anti-pattern for performance.
**Action:** Use the O(n) Fisher-Yates algorithm for in-place array shuffling instead. It is algorithmically faster, avoids unnecessary memory allocations, and provides an unbiased shuffle.
