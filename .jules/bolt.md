...
## 2024-03-26 - [Inefficient Array Shuffling]
**Learning:** Using chained `map().sort().map()` with `Math.random()` to shuffle arrays introduces O(n log n) overhead, allocates unnecessary intermediate arrays, and suffers from statistical bias.
**Action:** Use the Fisher-Yates algorithm for in-place, unbiased array shuffling in O(n) time, replacing the chained approach.
