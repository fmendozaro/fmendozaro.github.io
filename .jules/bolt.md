...
## 2024-05-18 - [Inefficient Array Shuffling]
**Learning:** Using `array.map().sort().map()` chains to shuffle an array introduces intermediate array allocations and O(n log n) sorting overhead, which is inefficient compared to in-place shuffling algorithms.
**Action:** Replace `map().sort().map()` sorting chains with the O(n) Fisher-Yates algorithm, which creates a single shallow copy and swaps elements in-place.
