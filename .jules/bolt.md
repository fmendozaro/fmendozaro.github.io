...
## 2024-05-15 - [Inefficient Array Shuffling Patterns]
**Learning:** Shuffling an array using `.map((a) => ({sort: Math.random(), value: a})).sort((a, b) => a.sort - b.sort).map((a) => a.value)` is extremely inefficient for performance. It introduces multiple intermediate object and array allocations, and uses an O(n log n) sorting algorithm simply to randomize order. This overhead is particularly noticeable in frontend loops where memory allocation and garbage collection can cause stutter.
**Action:** Use the Fisher-Yates algorithm for in-place array shuffling instead. It operates in O(n) time and does not require multiple intermediate array allocations.
