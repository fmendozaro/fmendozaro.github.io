...
## 2024-05-15 - [Inefficient Array Shuffling with .map().sort().map()]
**Learning:** Shuffling arrays using `.map().sort().map()` with `Math.random()` introduces O(n log n) overhead, multiple intermediate array allocations, and inherent statistical bias.
**Action:** Use the Fisher-Yates algorithm for O(n) in-place array shuffling instead. This reduces overhead and avoids unnecessary allocations.
