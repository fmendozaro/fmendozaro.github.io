...
## 2024-03-26 - [Array Shuffling Performance]
**Learning:** Using `map().sort().map()` chains with `Math.random()` for array shuffling introduces O(n log n) sorting overhead, allocates multiple intermediate arrays, and suffers from statistical bias. In codebases with such patterns, it creates unnecessary performance bottlenecks and garbage collection overhead.
**Action:** Replace `map().sort().map()` shuffling patterns with the Fisher-Yates algorithm, which provides an O(n) in-place shuffle. Ensure to clone the source array (e.g., `[...array]`) before shuffling to prevent unintended mutations.
