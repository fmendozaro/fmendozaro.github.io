...
## 2024-06-04 - [Array Shuffle Anti-pattern]
**Learning:** Shuffling an array using `.map().sort().map()` creates multiple intermediate arrays and excessive object allocation. It also has a time complexity of O(n log n). This is a common anti-pattern that slows down execution, especially when ran on client-side rendering where main thread blocking should be minimized.
**Action:** Use the O(n) Fisher-Yates shuffle algorithm which performs in-place shuffling without creating intermediate arrays.
