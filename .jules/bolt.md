...
## 2026-03-29 - Removing unused CSS classes in vendor libraries
**Learning:** For performance optimization, manually removing unused classes from vendor or library CSS files (like `css/hover.css`) can significantly reduce the file size and improve loading times, especially in projects without automated build steps (like PurgeCSS).
**Action:** When auditing CSS performance in projects without a build step, identify all used classes from large library files and trim the file to include only those necessary classes.
