...
## 2026-06-24 - [Inefficient Array Shuffle Anti-pattern]
**Learning:** The `map().sort().map()` chain pattern for shuffling arrays using `Math.random()` is an anti-pattern. It incurs O(n log n) time complexity, allocates multiple intermediate arrays, and introduces statistical bias.
**Action:** Always use the Fisher-Yates algorithm for in-place array shuffling to ensure O(n) performance and unbiased randomness, especially for arrays generated repeatedly on the client-side.
