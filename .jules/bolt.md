...
## 2024-05-15 - [Inefficient Array Shuffling]
**Learning:** Shuffling an array using `map((a) => ({sort: Math.random(), value: a})).sort((a, b) => a.sort - b.sort).map((a) => a.value)` is extremely inefficient. It involves O(n log n) sorting overhead, allocates three intermediate arrays, and suffers from statistical sorting bias.
**Action:** Use the O(n) Fisher-Yates algorithm for in-place array shuffling instead. Ensure the source array is cloned first (e.g., using `[...array]`) to prevent unintended mutation of the original data.
