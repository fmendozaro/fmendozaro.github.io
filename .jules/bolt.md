...
## 2026-06-14 - [Array Shuffling Performance Anti-pattern]
**Learning:** The `array.map().sort().map()` pattern using `Math.random()` for shuffling arrays has an O(n log n) overhead, involves multiple intermediate array allocations, and has inherent statistical bias. This is inefficient for performance-critical applications.
**Action:** Use the O(n) Fisher-Yates algorithm for in-place array shuffling to avoid unnecessary memory overhead and algorithmic bias.
