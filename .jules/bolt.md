...
## 2024-05-15 - [Inefficient Array Shuffling with Sort]
**Learning:** Using `map().sort().map()` chains with `Math.random()` for array shuffling introduces O(n log n) overhead, requires multiple intermediate array allocations, and has inherent statistical bias. This is an anti-pattern for performance and correctness.
**Action:** Always use the Fisher-Yates algorithm for in-place array shuffling instead. It is O(n), requires minimal memory allocation, and provides an unbiased shuffle.
